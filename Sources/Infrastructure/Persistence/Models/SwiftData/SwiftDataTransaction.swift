...
@Model
internal final class SwiftDataTransaction {
    ...
    /// リレーション
    internal var financialInstitution: SwiftDataFinancialInstitution?
    internal var majorCategory: SwiftDataCategory? // 大項目
    internal var minorCategory: SwiftDataCategory? // 中項目
    ...
    internal init(
        ...
        financialInstitution: SwiftDataFinancialInstitution? = nil,
        majorCategory: SwiftDataCategory? = nil,
        minorCategory: SwiftDataCategory? = nil,
    ) {
        self.id = id
        self.date = date
        self.title = title
...
internal extension SwiftDataTransaction {
    /// 支出かどうか（金額がマイナス）
    var isExpense: Bool {
        amount < 0
    }
