...
@Suite("SwiftDataTransaction Initialization Tests")
internal struct TransactionInitializationTests {
    ...
    internal func initializeTransactionWithAllParameters() {
        ...
        let majorCategory = SwiftDataCategory(name: "食費")
        let minorCategory = SwiftDataCategory(name: "外食", parent: majorCategory)
...
@Suite("SwiftDataTransaction Computed Property Tests")
internal struct TransactionComputedPropertyTests {
    ...
    internal func absoluteAmount() {
        let transaction1 = SwiftDataTransaction(date: Date(), title: "支出", amount: -1000)
        let transaction2 = SwiftDataTransaction(date: Date(), title: "収入", amount: 5000)
    ...
    internal func categoryFullNameWithMinorSwiftDataCategory() {
...
@Suite("SwiftDataTransaction Validation Tests")
internal struct TransactionValidationTests {
    ...
    internal func validateMinorCategoryOnly() {
        let minor = SwiftDataCategory(name: "外食")
    ...
    internal func validateCategoryMismatch() {
        let major1 = SwiftDataCategory(name: "食費")
        let major2 = SwiftDataCategory(name: "日用品")
        let minor = SwiftDataCategory(name: "外食", parent: major1)
...
@Suite("SwiftDataTransaction Flag Tests")
internal struct TransactionFlagTests {
    @Test("計算対象フラグを設定できる")
    internal func checkIsIncludedInCalculationFlag() {
        let transaction1 = SwiftDataTransaction(
            date: Date(),
            title: "テスト1",
            amount: -1000,
            isIncludedInCalculation: true,
        )
        let transaction2 = SwiftDataTransaction(
            date: Date(),
            title: "テスト2",
            amount: -1000,
...
@Suite("SwiftDataTransaction Timestamp Tests")
