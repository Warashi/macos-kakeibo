...
internal struct AnnualBudgetPanelTests {
    ...
    internal func annualBudgetPanelInitialization() {
        ...
        let allocation = SwiftDataAnnualBudgetAllocation(amount: 60000, category: category)
    ...
    internal func contentBuilderReflectsUsage() {
        let category = SwiftDataCategory(name: "特別費", allowsAnnualBudget: true)
        let config = SwiftDataAnnualBudgetConfig(year: 2025, totalAmount: 100_000, policy: .automatic)
        let allocation = SwiftDataAnnualBudgetAllocation(amount: 60000, category: category, policyOverride: .manual)
        allocation.config = config
        config.allocations = [allocation]
    ...
    internal func contentBuilderHandlesMissingUsage() {
        let category = SwiftDataCategory(name: "特別費", allowsAnnualBudget: true)
        let config = SwiftDataAnnualBudgetConfig(year: 2025, totalAmount: 80000, policy: .automatic)
        let allocation = SwiftDataAnnualBudgetAllocation(amount: 50000, category: category)
        allocation.config = config
        config.allocations = [allocation]
